...
async function legacyHandler(request, response) {
  ...
  if (slug) {
    await handleItem(request, response, "projects", slug);
    return;
  }

  await handleCollection(request, response, "projects");
}

exports.handler = withLegacyHandler("projects", legacyHandler);
