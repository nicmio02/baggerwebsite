...
async function readBlobStore(config) {
  const raw = await getCmsStore().get(config.key, { type: "text" });
...
async function writeStore(type, items) {
  ...
  if (!config) {
    throw new Error("Collectie niet gevonden.");
  }

  await getCmsStore().setJSON(config.key, items);
}
...
function sendMutationError(response, error) {
  ...
  const statusCode = /blobs?/i.test(message) ? 501 : 400;
